...
#[tauri::command]
pub async fn get_config_text(vip:bool) -> CmdResult {
    let item = wrap_err!(PrfItem::from_free_url(vip).await)?;
...
#[tauri::command]
pub async fn disable_system_proxy() -> CmdResult {
    handle::Handle::disable_system_proxy();
    Ok(())
}

#[tauri::command]
pub async fn enable_system_proxy() -> CmdResult {
    handle::Handle::enable_system_proxy();
    Ok(())
}

#[tauri::command]
pub async fn refresh_userinfo() -> CmdResult {
    handle::Handle::refresh_userinfo();
    Ok(())
}
#[tauri::command]
pub async fn get_base_url() -> CmdResult<String> {
    let item = wrap_err!(PrfItem::get_base_url().await)?;
    Ok(item.url.unwrap())
}
#[tauri::command]
pub async fn get_apple_time() -> CmdResult<String> {
    let item = wrap_err!(PrfItem::get_apple_time().await)?;
    Ok(item.url.unwrap())
}

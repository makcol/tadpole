...
pub fn init_config() -> Result<()> {
    ...
    crate::log_err!(dirs::clash_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IClashTemp::template().0, Some("# Tadpole VPN"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::verge_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IVerge::template(), Some("# Tadpole VPN"))?;
        }
        <Result<()>>::Ok(())
    }));
    ...
    crate::log_err!(dirs::profiles_path().map(|path| {
        if !path.exists() {
            help::save_yaml(&path, &IProfiles::template(), Some("# Tadpole VPN"))?;
        }
        <Result<()>>::Ok(())
    }));

...
async function resolve() {
  ...
  const appPathList = [
    join("macos/Tadpole VPN.aarch64.app.tar.gz"),
    join("macos/Tadpole VPN.aarch64.app.tar.gz.sig"),
  ];
  ...
  fs.copyFileSync(join("macos/Tadpole VPN.app.tar.gz"), appPathList[0]);
  fs.copyFileSync(join("macos/Tadpole VPN.app.tar.gz.sig"), appPathList[1]);
  ...
  await uploadAssets(release.id, [
    join(`dmg/Tadpole VPN_${version}_aarch64.dmg`),
    ...appPathList,
  ]);
}
